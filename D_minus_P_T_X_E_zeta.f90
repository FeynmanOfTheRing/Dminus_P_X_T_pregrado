module subrutinas_funciones_parametros
    ...
    real(8):: d=2d0, R1=1d0, R2=1d0,dseta=1d0,R,ds_ini=-1d0,ds_fin=3d0,ds_step=0.1d0
    real(8):: mu,Etol,Er,En,En_ant,Uee,Ue1d,Ue2d,phi,tol,Fdospi,Fcero,F,Fant,yini!masa reducida, energías, potenciales,to
    real(8)::X=0d0,P=0,Pfin,Pini,Pstep,T=4d0 
    real(16)::cte_norm
        ...
        Do while(En.le.Efin) !Ciclo que varia las energías
            ...
            if((F*Fant.lt.0d0)) then
                ...
                if(abs(Fcero-Fdospi) .lt. tol) then
                    ...
                    WRITE(*,*) Ue1d,Ue2d
                    if(k.eq.1) then
                        write(4,*) Etol,dseta !escribo los valores E vs R !cAMBIAR
                        write(*,*) Etol,dseta ,"k=1"
                        bandera_imprimir=1
                        call Rungekutta_anillo
                    end if
                    if(k.eq.2) then
                        write(3,*) Etol,dseta!escribo los valores E vs R
                        write(*,*) Etol,dseta,"k=2"
                        bandera_imprimir=2
                        call Rungekutta_anillo
                    end if              
                    if(k.eq.3) then
                        write(5,*) Etol,dseta !escribo los valores E vs R
                        write(*,*) Etol,dseta ,"k=3"
                        bandera_imprimir=3
                        call Rungekutta_anillo
                        exit
                    end if
                    ...
                    if(k.eq.1) then
                        write(4,*) Etol,dseta !escribo los valores E vs R
                        write(*,*) Etol,dseta ,"k=1"
                        bandera_imprimir=1
                        call Rungekutta_anillo
                    end if
